...
def train(
    ...
    if num_gpus > 0:
        ctx = [mx.gpu(i)
               for i in range(num_gpus)] if num_gpus > 1 else mx.gpu()
    else:
        ctx = mx.cpu()
    ...
    if network == 'unet':
        ...
        sym = build_enet(
            inp_dims=train_iter.provide_data[0][1][1:], num_classes=num_classes, class_weights=class_weights)
    print "Sym loaded"
    net = mx.mod.Module(
        sym, context=ctx, data_names=(
            'data',), label_names=(
            'label',))
    dice_metric = mx.metric.CustomMetric(
        feval=avg_dice_coef_metric,

...
def conv_block(inp, num_filter, kernel, pad, block, conv_block):
    conv = mx.sym.Convolution(
        ...
        (block,
         conv_block))
    conv = mx.sym.BatchNorm(
        conv, fix_gamma=True, name='bn%i_%i' %
        (block, conv_block))
    conv = mx.sym.Activation(
        conv, act_type='relu', name='relu%i_%i' %
        (block, conv_block))
    return conv
...
def down_block(inp, num_filter, kernel, pad, block, pool=True):
    ...
    if pool:
        pool = mx.sym.Pooling(
            conv, kernel=(
                2, 2), stride=(
                2, 2), pool_type='max', name='pool_%i' %
            block)
        return pool, conv
    return conv
...
def down_branch(inp):
    pool1, conv1 = down_block(
        inp, num_filter=32, kernel=(
            3, 3), pad=(
            1, 1), block=1)
    pool2, conv2 = down_block(
        pool1, num_filter=64, kernel=(
            3, 3), pad=(
            1, 1), block=2)
    pool3, conv3 = down_block(
        pool2, num_filter=128, kernel=(
            3, 3), pad=(
            1, 1), block=3)
    pool4, conv4 = down_block(
        pool3, num_filter=256, kernel=(
            3, 3), pad=(
            1, 1), block=4)
    conv5 = down_block(
        pool4, num_filter=512, kernel=(
            3, 3), pad=(
            1, 1), block=5, pool=False)
    return [conv5, conv4, conv3, conv2, conv1]


def up_block(inp, down_feature, num_filter, kernel, pad, block):
    trans_conv = mx.sym.Deconvolution(
        inp, num_filter=num_filter, kernel=(
            2, 2), stride=(
            2, 2), no_bias=True, name='trans_conv_%i' %
        block)
    up = mx.sym.concat(*[trans_conv, down_feature],
                       dim=1, name='concat_%i' % block)
...
def up_branch(down_features, num_classes):
    conv6 = up_block(
        ...
        block=6)
    conv7 = up_block(
        conv6, down_features[2], num_filter=128, kernel=(
            3, 3), pad=(
            1, 1), block=7)
    conv8 = up_block(
        conv7, down_features[3], num_filter=64, kernel=(
            3, 3), pad=(
            1, 1), block=8)
    conv9 = up_block(
        conv8, down_features[4], num_filter=32, kernel=(
            3, 3), pad=(
            1, 1), block=9)
    conv10 = mx.sym.Convolution(
        conv9, num_filter=num_classes, kernel=(
            1, 1), name='conv10_1')
    return conv10
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        if is_train:
            self._spatial_dropout_mask = F.broadcast_greater(
                F.random_uniform(
                    low=0, high=1, shape=(
                        1, self._num_filters, 1, 1), ctx=self._ctx), F.ones(
                    shape=(
                        1, self._num_filters, 1, 1), ctx=self._ctx) * self._p, ctx=self._ctx)
            y = F.broadcast_mul(x,
                                self._spatial_dropout_mask,
                                ctx=self._ctx) / (1 - self._p)
...
def initial_block(
    ...
    padded_inp = mx.sym.pad(
        inp, mode='constant', pad_width=same_padding(
            inp_dims, outp_dims, strides, kernel=(
                nb_row, nb_col)), name='init_pad')
    conv = mx.sym.Convolution(
        padded_inp,
        ...
        stride=strides,
        name='init_conv')
    max_pool = mx.sym.Pooling(
        inp, kernel=(
            2, 2), stride=(
            2, 2), pool_type='max', name='init_pool')
    merged = mx.sym.concat(*[conv, max_pool], dim=1, name='init_concat')
    return merged


def encoder_bottleneck(
        inp,
        inp_filter,
        output,
        name,
    ...
    encoder = mx.sym.Convolution(
        encoder, num_filter=internal, kernel=(
            input_stride, input_stride), stride=(
            input_stride, input_stride), no_bias=True, name="conv1_%i" %
        name)
    # Batch normalization + PReLU
    encoder = mx.sym.BatchNorm(encoder, momentum=0.1, name="bn1_%i" % name)
    encoder = mx.sym.LeakyReLU(
        encoder,
        act_type='prelu',
        name='prelu1_%i' %
        name)
    ...
    if not asymmetric and not dilated:
        ...
        encoder = mx.sym.Convolution(
            encoder, num_filter=internal, kernel=(
                1, asymmetric), pad=(
                0, asymmetric // 2), no_bias=True, name="conv3_%i" %
            name)
        encoder = mx.sym.Convolution(
            encoder, num_filter=internal, kernel=(
                asymmetric, 1), pad=(
                asymmetric // 2, 0), name="conv4_%i" %
            name)
    elif dilated:
        encoder = mx.sym.Convolution(encoder,
                                     num_filter=internal,
                                     kernel=(3,
                                             3),
                                     dilate=(dilated,
    ...
    # other branch
    if downsample:
        other = mx.sym.Pooling(
            other, kernel=(
                2, 2), stride=(
                2, 2), pool_type='max', name='pool1_%i' %
            name)
        other = mx.sym.transpose(
            other, axes=(
                0, 2, 1, 3), name='trans1_%i' %
            name)
...
def build_encoder(inp, inp_dims, dropout_rate=0.01):
    ...
    enet = encoder_bottleneck(
        ...
        128,
        downsample=True,
        name=19)  # bottleneck 2.0
    # bottleneck 2.x and 3.x
    for n in range(2):
        enet = encoder_bottleneck(
            enet, 128, 128, name=n * 10 + 20)  # bottleneck 2.1
        enet = encoder_bottleneck(
            enet,
            128,
            128,
        ...
        enet = encoder_bottleneck(
            ...
            23)  # bottleneck 2.4
        enet = encoder_bottleneck(
            enet, 128, 128, name=n * 10 + 24)  # bottleneck 2.5
        enet = encoder_bottleneck(
            enet,
            128,
            128,
...
def decoder_bottleneck(
    ...
    if not upsample:
        x = mx.sym.Convolution(
            x, num_filter=internal, kernel=(
                3, 3), pad=(
                1, 1), no_bias=False, name="conv7_%i" %
            name)
    else:
        x = mx.sym.Deconvolution(
            x, num_filter=internal, kernel=(
                3, 3), stride=(
                2, 2), target_shape=upsample_dims, name="dconv1_%i" %
            name)
...
def build_decoder(encoder, nc, output_shape=(3, 512, 512)):
    ...
    enet = decoder_bottleneck(
        ...
        upsample_dims=(
            ...
            output_shape[2] // 2),
        reverse_module=True,
        name=23)  # bottleneck 5.0
    enet = decoder_bottleneck(enet, 16, 16, name=24)  # bottleneck 5.1

    enet = mx.sym.Deconvolution(
        enet, num_filter=nc, kernel=(
            2, 2), stride=(
            2, 2), target_shape=(
                output_shape[1], output_shape[2]), name='dconv2')
    return enet
